// frontend/src/components/SystemCard.tsx
...
export const SystemCard: React.FC<Props> = ({
  ...
  return (
    ...
    >
      <div
        className={`flex flex-col xl:flex-row xl:justify-between xl:items-start gap-6 ${!isCollapsed ? "mb-8" : ""}`}
      >
        {/* WH INFO SECTION */}
          ...
          <div className="flex items-start gap-3">
            <div className="bg-sky-500/10 border border-sky-500/30 px-4 py-2 rounded-lg inline-block self-start relative">
              <div className="flex items-center gap-2 mb-1">
                {wh.id === 31000302 && (
                  <span className="text-[10px] block uppercase font-black text-sky-500 tracking-widest leading-none">
                    Home
                  </span>
                )}
            ...
            >
              <ChevronDown
                size={20}
                className={`transform transition-transform duration-200 ${isCollapsed ? "-rotate-90" : "rotate-0"}`}
              />
            </button>
          </div>
          ...
          <div className="flex flex-wrap gap-2 items-center xl:ml-[44px]">
            {anoikData.systems[wh.name]?.statics?.map((staticCode: string) => {
              ...
              return (
                <span
                  key={staticCode}
                  className="text-[10px] bg-slate-800/50 text-slate-400 px-2 py-1 rounded border border-slate-700 font-mono"
                >
                  Static{" "}
                  <strong className="text-slate-200">{staticCode}</strong>
